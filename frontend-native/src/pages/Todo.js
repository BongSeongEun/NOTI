...
function Todo() {
  ...
  const generateGoogleCalendarURL = event => {
    ...
    const startDateTime = new Date(`${selectedDate}T${event.todoStartTime}:00Z`)
      .toISOString()
      .replace(/[-:.]/g, '')
      .slice(0, -1);
    const endDateTime = new Date(`${selectedDate}T${event.todoEndTime}:00Z`)
      .toISOString()
      .replace(/[-:.]/g, '')
      .slice(0, -1);
    ...
    <ThemeProvider theme={currentTheme}>
      <FullView>
        <MainView>
          <HorisontalView style={{marginTop: 20, marginBottom: 10}}>
            <Profile
              source={base64Image ? {uri: base64Image} : images.profile}
              style={{marginTop: 20}}
            />
      ...
      <FullView style={{flex: 1, marginBottom: 80}}>
        <BarContainer>
          ...
          <MainText
            onPress={() => navigation.navigate('Coop_Main')}
            style={{marginLeft: 20, color: '#B7BABF'}}>
            협업 일정
          </MainText>
        </BarContainer>
        <Bar />
        ...
        <ScrollView>
          <MainView>
            ...
            {events.length === 0 ? (
              ...
              events.map((event, index) => (
                <Noti
                  key={event.todoId}
                  style={{
                    backgroundColor: event.todoDone
                      ? addOpacityToColor(event.selectedColor, 0.6)
                      : event.selectedColor,
                  }}
                  onPress={() => {
                    setModalVisible(true);
                  ...
                  <Noti_Check
                    onPress={() => toggleComplete(event.todoId, index)}>
                    {event.todoDone && (
                      <images.noticheck
                        width={15}
                        height={15}
            ...
            <Noti
              onPress={() =>
                navigation.navigate('Todo_Add', {selectedDate: selectedDate})
              }
              style={{
                width: 150,
              ...
              <ModalContainer>
                <ModalView>
                  <ModalContent>
                    ...
                    <TouchableOpacity
                      onPress={() => setClicked_delete(true)}
                      style={{padding: 10}}>
                    ...
                    <Modal
                      ...
                      <ModalContainer>
                        <ModalView>
                          <MainText style={{margin: 20, fontSize: 15}}>
                            정말 삭제하시겠습니까?
                          </MainText>
                          <HorisontalView
                            style={{
                            ...
                            <TeamOut
                              onPress={() => setClicked_delete(false)}
